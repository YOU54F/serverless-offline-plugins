...
class ServerlessDynamoDBOfflinePlugin {
  ...
  public constructor(private serverless: Serverless) {
    ...
    this.commands = {
      ...
      //     start: {
      //       usage: "startDynamoDB.",
      //       lifecycleEvents: ["startDynamoDB"],
      //     },
      //     stop: {
      //       usage: "stopDynamoDB.",
      //       lifecycleEvents: ["stopDynamoDB"],
      //     },
      //   },
      // },
    };
    ...
    this.hooks = {
      "before:offline:end": this.stopDynamoDB,
      "before:offline:start": this.startDynamoDB,
    };
  }
  ...
  private shouldExecute = () => {
    if (
      this.dynamoDBConfig.stages &&
      this.dynamoDBConfig.stages.includes(
  ...
  private startDynamoDB = async () => {
    ...
    Object.values({
      ...
      if (stack instanceof Array) {
        stack.forEach((stackResource: Stack) => {
          if (stackResource.Resources) {
            Object.values(stackResource.Resources).forEach((resource: any) => {
    ...
    this.serverless.cli.log(
      "DynamoDB Offline - found dynamo resources to create" +
